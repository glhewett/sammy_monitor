...
use sammy_monitor::metrics::{MonitorMetadata, METRICS_REGISTRY, init_metrics};
...
async fn test_metrics_integration() {
    ...
    let handle = metrics_exporter_prometheus::PrometheusBuilder::new()
        ...
        .set_buckets_for_metric(
            metrics_exporter_prometheus::Matcher::Full("http_monitor_response_time_seconds".to_string()),
    ...
    assert!(!output.trim().is_empty(), "Metrics output should not be empty");
    
    // Test 2: Check that all expected metric types are present
    assert!(output.contains("http_monitor_response_time_seconds"), "Should contain response time metrics");
    assert!(output.contains("http_monitor_requests_total"), "Should contain request total counters");
    assert!(output.contains("http_monitor_failures_total"), "Should contain failure counters");
    assert!(output.contains("http_monitor_up"), "Should contain monitor status gauges");
    assert!(output.contains("http_monitor_last_success_timestamp"), "Should contain last success timestamps");
    ...
    assert_eq!(monitor1_success, Some(6.0), "Monitor 1 should have 6 successful requests");
    ...
    assert_eq!(monitor2_failure, Some(3.0), "Monitor 2 should have 3 failed requests");
    ...
    assert_eq!(monitor2_success, Some(1.0), "Monitor 2 should have 1 successful request");
    ...
    assert_eq!(http_404_failures, Some(1.0), "Should have 1 HTTP 404 failure");
    ...
    assert_eq!(http_500_failures, Some(1.0), "Should have 1 HTTP 500 failure");
    ...
    if let Some(sum) = histogram_sum {
        assert!((sum - expected_sum_seconds).abs() < 0.001, 
                "Monitor 1 histogram sum should be {} but got {}", expected_sum_seconds, sum);
    } else {
    ...
    assert_eq!(histogram_count, Some(6.0), "Monitor 1 histogram should have 6 observations");
    ...
    assert_eq!(monitor2_status, Some(1.0), "Monitor 2 should be UP (last op was success edge case)");
    ...
    if let Some(timestamp) = monitor1_timestamp {
        ...
        assert!(current_time - timestamp < 10.0, 
                "Monitor 1 last success timestamp should be recent: {} vs {}", timestamp, current_time);
    } else {
...
fn extract_metric_value(output: &str, metric_name: &str, labels: &[(&str, &str)]) -> Option<f64> {
    for line in output.lines() {
        ...
        if line.starts_with(metric_name) {
            ...
            let all_labels_match = labels.iter().all(|(key, value)| {
...
fn validate_histogram_bucket(output: &str, monitor_id: &Uuid, le_value: &str, expected_count: f64) {
    ...
    assert_eq!(bucket_count, Some(expected_count), 
               "Bucket le=\"{}\" should have {} observations but got {:?}", 
               le_value, expected_count, bucket_count);
}
